...
import org.keedio.storm.bolt.filter.SerializeBolt;
...
public class MetricsController implements Serializable {
    ...
    public void startGangliaMetrics(String server, int port) {
    	try {

    		final GMetric ganglia = new GMetric(server, port, UDPAddressingMode.UNICAST, 1);
    		...
    		LOG.error("Couldn´t start gangliareporter on server " + server + ":" + port, e);
    	}
    }
